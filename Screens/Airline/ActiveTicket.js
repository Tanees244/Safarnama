...
const ActiveTicket = ({
  ticketDetails: initialTicketDetails,
  ticketNumber,
  onUpdateTicket,
  onDeleteTicket,
}) => {
  ...
  const onChange = (event, selectedDate) => {
    if (selectedDate) {
      ...
      setEditedTicket({ ...editedTicket, departureDate: currentDate });
      if (Platform.OS === "android") {
        toggleDatepicker();
  ...
  const showDatePicker = () => {
    return (
      <DateTimePicker
        mode="date"
        display="compact"
        value={editedTicket.departureDate || new Date()} 
        onChange={onChange}
      />
    );
  };
  ...
  const handleSave = async () => {
    try {
      const response = await fetch(
        `http://192.168.100.12:8000/api/VendorsRoutes/update_airline_package/${editedTicket.airline_operations_id}`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelete = async () => {
    try {
      const response = await fetch(
        `http://192.168.100.12:8000/api/VendorsRoutes/delete_airline_package/${ticketDetails.airline_operations_id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <View style={[styles.activeTicketContainer, { width: containerWidth }]}>
      ...
      >
        Ticket # {ticketDetails.airline_operations_id}
      </Text>
      ...
      <View style={styles.flightInfoContainer}>
        <Text style={styles.flightNumber}>
          {" "}
          Flight # {ticketDetails.flight_number}
        </Text>
      </View>
      ...
      <View style={styles.seatTypeContainer}>
        <View
          style={[
            ...
            ticketDetails.seat_type &&
              ticketDetails.seat_type.trim().toLowerCase() === "first-class" &&
              styles.selectedSeat,
          ]}
        >
        ...
        <View
          style={[
            styles.seatTypeOption,
            ticketDetails.seat_type &&
              ticketDetails.seat_type.trim().toLowerCase() === "business" &&
              styles.selectedSeat,
          ]}
        >
        ...
        <View
          style={[
            styles.seatTypeOption,
            ticketDetails.seat_type &&
              ticketDetails.seat_type.trim().toLowerCase() === "economy" &&
              styles.selectedSeat,
          ]}
        >
            ...
            <View style={styles.DTime}>
              ...
              <Text style={styles.ticketText2}>
                {(ticketDetails.departure_date &&
                  new Date(ticketDetails.departure_date)?.toDateString()) ||
                  " "}
              </Text>
              <Text style={styles.title2}>Departure Time : </Text>
              <Text style={styles.ticketText2}>
                {(ticketDetails.departure_time &&
                  formatTime(ticketDetails.departure_time)) ||
                  " "}
              </Text>
            </View>
            <View style={styles.ATime}>
              ...
              <Text style={styles.ticketText2}>
                {(ticketDetails.arrival_date &&
                  new Date(ticketDetails.arrival_date)?.toDateString()) ||
                  " "}
              </Text>
              <Text style={styles.title2}>Arrival Time : </Text>
              <Text style={styles.ticketText2}>
                {(ticketDetails.arrival_time &&
                  formatTime(ticketDetails.arrival_time)) ||
                  " "}
              </Text>
            </View>
          </View>
          <View style={styles.Duration}>
            <View>
              ...
              <Text style={styles.ticketText}>
                {ticketDetails.ticket_price}PKR
              </Text>
            </View>
            <View>
      ...
      <Modal visible={isEditModalVisible} animationType="slide">
        <View style={styles.modalContent}>
            ...
            {!showPicker && (
              <Pressable onPress={toggleDatepicker}>
                <TextInput
                  ...
                  value={
                    dateSelect ||
                    editedTicket.departureDate?.toDateString() ||
                    ""
                  }
                  editable={false}
                />
            ...
            {!showPicker && (
              <Pressable onPress={toggleDatepicker}>
                <TextInput
                  ...
                  value={
                    dateSelect || editedTicket.arrivalDate?.toDateString() || ""
                  }
                  editable={false}
                />
              </Pressable>
          ...
          <TextInput
            ...
            placeholder="Price"
            value={
              editedTicket.price !== undefined
                ? editedTicket.price.toString()
                : ""
            }
