...
const Flight = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          "http://192.168.100.18:8000/api/railway-packages/"
        );
  ...
  const handleSearch = () => {
    ...
    const filteredRailwayTickets = airline.filter(
      (airline) =>
        (!lowerCaseDepartureCity ||
          airline.departure_city
            .toLowerCase()
            .includes(lowerCaseDepartureCity)) &&
        (!lowerCaseArrivalCity ||
          airline.arrival_city.toLowerCase().includes(lowerCaseArrivalCity)) &&
        (!lowerCaseDepartureDate ||
          airline.departure_date
            .toLowerCase()
            .includes(lowerCaseDepartureDate)) &&
        (!lowerCaseReturnDate ||
          airline.arrival_date.toLowerCase().includes(lowerCaseReturnDate))
    );

    setFilteredTickets([...filteredRailwayTickets]);
  };
        ...
        <View style={styles.searchInputs}>
          <View style={styles.inputRow}>
            <TextInput
              ...
              onChangeText={(text) =>
                setSearchInput({ ...searchInput, destinationPlace: text })
              }
            />
            <TextInput
              style={styles.input}
              ...
              onChangeText={(text) =>
                setSearchInput({ ...searchInput, arrivalPlace: text })
              }
            />
          </View>
          ...
          <View style={styles.inputRow}>
            <TextInput
              ...
              onChangeText={(text) =>
                setSearchInput({ ...searchInput, departureDate: text })
              }
            />
            <TextInput
              style={styles.input}
