...
import ActiveTicket from './ActiveTicketT';
...
const TrainDashboard = () => {
  ...
  const ProfileNavigate = () => {
    navigation.navigate('TrainProfile');
  }
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch("http://192.168.100.12:8000/api/VendorsRoutes/railway_packages", {
        method: "POST",
        headers: {
  ...
  const fetchUserProfile = async (token) => {
    try {
      ...
      const response = await fetch(
        "http://192.168.100.12:8000/api/VendorsRoutes/railway-details/",
        {
          headers: {
  ...
  const fetchAirlinePackages = async () => {
    try {
      const authToken = await AsyncStorage.getItem("authToken");
      const response = await fetch("http://192.168.100.12:8000/api/VendorsRoutes/get_railway_packages", {
        headers: {
          "Authorization": `Bearer ${authToken}`,
        },
      });
      if (response.ok) {
        ...
        console.error("Failed to fetch railway packages:", response.status);
      }
    } catch (error) {
